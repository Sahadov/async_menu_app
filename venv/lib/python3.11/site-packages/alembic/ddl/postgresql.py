...
class CreateExcludeConstraintOp(ops.AddConstraintOp):
    ...
    def __init__(
        ...
        where: BinaryExpression | str | None = None,
        schema: str | None = None,
...
def _exclude_constraint(
    ...
    if alter:
        ...
        return '{prefix}create_exclude_constraint({args})'.format(
            prefix=render._alembic_autogenerate_prefix(autogen_context),
            args=', '.join(args),
        )
    else:
        args = do_expr_where_opts()
        return '{prefix}ExcludeConstraint({args})'.format(
