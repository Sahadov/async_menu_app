...
select @const_name = QUOTENAME([name]) from {type}
where parent_object_id = object_id('{schema_dot}{tname}')
and col_name(parent_object_id, parent_column_id) = '{colname}'
exec('alter table {tname_quoted} drop constraint ' + @const_name)""".format(
        type=type_,
        tname=tname,
        colname=colname,
...
where fkc.parent_object_id = object_id('{schema_dot}{tname}')
and col_name(fkc.parent_object_id, fkc.parent_column_id) = '{colname}'
exec('alter table {tname_quoted} drop constraint ' + @const_name)""".format(
        tname=tname,
        colname=colname,
        tname_quoted=format_table_name(compiler, tname, schema),
...
def visit_rename_column(
    ...
    return "EXEC sp_rename '{}.{}', {}, 'COLUMN'".format(
