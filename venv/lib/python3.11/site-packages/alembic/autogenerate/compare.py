...
def _compare_tables(
    ...
    for s, tname in metadata_table_names.difference(conn_table_names):
        name = f'{s}.{tname}' if s else tname
...
def _make_index(params: dict[str, Any], conn_table: Table) -> Index | None:
...
def _compare_columns(
    ...
    name = f'{schema}.{tname}' if schema else tname
...
def _compare_indexes_and_uniques(
    ...
    for removed_name in sorted(set(conn_names).difference(metadata_names)):
        conn_obj: _ix_constraint_sig | _uq_constraint_sig = conn_names[
            removed_name
        ]
    ...
    for existing_name in sorted(set(metadata_names).intersection(conn_names)):
        ...
        if conn_obj.is_index != metadata_obj.is_index:
            ...
            if conn_obj.sig != metadata_obj.sig:
                msg.append(
                    f' expression {conn_obj.sig!r} to {metadata_obj.sig!r}'
                )
...
def _warn_computed_not_supported(tname: str, cname: str) -> None:
    util.warn(f'Computed default on {tname}.{cname} cannot be modified')
