...
def _add_table(autogen_context: AutogenContext, op: ops.CreateTableOp) -> str:
    ...
    text = '{prefix}create_table({tablename!r},\n{args}'.format(
...
def _drop_table(autogen_context: AutogenContext, op: ops.DropTableOp) -> str:
    text = '{prefix}drop_table({tname!r}'.format(
        prefix=_alembic_autogenerate_prefix(autogen_context),
...
def _add_fk_constraint(
    ...
    return '{prefix}create_foreign_key({args})'.format(
        prefix=_alembic_autogenerate_prefix(autogen_context),
        args=', '.join(args),
    )
...
def _alter_column(
    ...
    if existing_type is not None:
        text += ',\n{}existing_type={}'.format(
            indent,
            _repr_type(existing_type, autogen_context),
        )
    if server_default is not False:
        rendered = _render_server_default(server_default, autogen_context)
        text += f',\n{indent}server_default={rendered}'

    if type_ is not None:
        text += f',\n{indent}type_={_repr_type(type_, autogen_context)}'
    if nullable is not None:
        text += f',\n{indent}nullable={nullable!r}'
    if comment is not False:
        text += f',\n{indent}comment={comment!r}'
    if existing_comment is not None:
        text += f',\n{indent}existing_comment={existing_comment!r}'
    if nullable is None and existing_nullable is not None:
        text += f',\n{indent}existing_nullable={existing_nullable!r}'
    if autoincrement is not None:
        text += f',\n{indent}autoincrement={autoincrement!r}'
    if server_default is False and existing_server_default:
        rendered = _render_server_default(
            existing_server_default, autogen_context
        )
        text += f',\n{indent}existing_server_default={rendered}'
    if schema and not autogen_context._has_batch:
        text += f',\n{indent}schema={schema!r}'
    text += ')'
    return text
...
class _f_name:
    ...
    def __repr__(self) -> str:
        return f'{self.prefix}f({_ident(self.name)!r})'
...
def _uq_constraint(
    ...
    if alter:
        ...
        args.extend([f'{k}={v!r}' for k, v in opts])
        return '{prefix}create_unique_constraint({args})'.format(
            prefix=_alembic_autogenerate_prefix(autogen_context),
            args=', '.join(args),
        )
    else:
        ...
        args.extend([f'{k}={v!r}' for k, v in opts])
        return '{prefix}UniqueConstraint({args})'.format(
            prefix=_sqlalchemy_autogenerate_prefix(autogen_context),
            args=', '.join(args),
        )
...
def _render_column(
    ...
    return '{prefix}Column({name!r}, {type}, {args}{kwargs})'.format(
        ...
        kwargs=(
            ', '.join(
                [f'{kwname}={val}' for kwname, val in opts]
                + [
                    '%s=%s'
                    % (key, _render_potential_expr(val, autogen_context))
...
def _render_computed(
    ...
    return '{prefix}Computed({text}, {kwargs})'.format(
        prefix=_sqlalchemy_autogenerate_prefix(autogen_context),
        text=text,
...
def _render_identity(
    ...
    return '{prefix}Identity({kwargs})'.format(
        prefix=_sqlalchemy_autogenerate_prefix(autogen_context),
        kwargs=(', '.join('%s=%s' % pair for pair in kwargs.items())),
    )
...
def _render_primary_key(
    ...
    return '{prefix}PrimaryKeyConstraint({args})'.format(
        prefix=_sqlalchemy_autogenerate_prefix(autogen_context),
        args=', '.join(
...
def _fk_colspec(
    ...
    if metadata_schema is not None and len(tokens) == 2:
        table_fullname = f'{metadata_schema}.{tname}'
    else:
    ...
    colspec = f'{table_fullname}.{colname}'
...
def _render_check_constraint(
    ...
    return '{prefix}CheckConstraint({sqltext}{opts})'.format(
        prefix=_sqlalchemy_autogenerate_prefix(autogen_context),
        opts=', ' + (', '.join(f'{k}={v}' for k, v in opts))
        if opts
        else '',
