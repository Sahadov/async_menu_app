...
class _GenerateRenderMethod:
    ...
    def write_variable_declares(self, identifiers, toplevel=False, limit=None):
        ...
        for ident in to_write:
            if ident in comp_idents:
                ...
                self.printer.writeline(
                    f'{ident} = _mako_get_namespace(context, {ident!r})'
                )
            else:
                if getattr(self.compiler, 'has_ns_imports', False):
                    ...
                    if self.compiler.strict_undefined:
                        ...
                        self.printer.writeline(
                            f'{ident} = context.get({ident!r}, UNDEFINED)'
                        )
    ...
    def write_def_decl(self, node, identifiers):
        ...
        self.printer.writeline('def {}({}):'.format(funcname, ','.join(namedecls)))
        self.printer.writeline(
            'return render_{}({})'.format(funcname, ','.join(nameargs))
        )
        self.printer.writeline(None)
    ...
    def write_inline_def(self, node, identifiers, nested):
        ...
        self.printer.writeline(
            'def {}({}):'.format(node.funcname, ','.join(namedecls))
        )
        filtered = len(node.filter_args.args) > 0
        buffered = eval(node.attributes.get('buffered', 'False'))
        cached = eval(node.attributes.get('cached', 'False'))
    ...
    def write_cache_decorator(
        ...
        self.printer.writeline(f'__M_{name} = {name}')
        ...
        self.printer.writeline('def {}({}):'.format(name, ','.join(args)))
        ...
        if buffered:
            s = (
                ...
                % (
                    ...
                    ','.join(pass_args),
                    ''.join(
                        [f'{k}={v}, ' for k, v in cache_args.items()]
                    ),
                    name,
                )
            )
    ...
    def visitBlockTag(self, node):
        if node.is_anonymous:
            ...
            self.printer.writeline(
                "context['self'].{}({})".format(node.funcname, ','.join(nameargs))
            )
            self.printer.writeline('\n')
